...
// State
const selectedTeacherId = ref(store.teachers[0]?.id); // Default to first teacher
const selectedVersionId = ref(store.activeVersions[0]?.id);

// Computed
const scheduleGrid = computed(() => {
    if (!selectedTeacherId.value || !selectedVersionId.value) return {};
    return store.getTeacherScheduleGrid(selectedTeacherId.value, selectedVersionId.value);
});
            ...
            <div class="flex items-center gap-4">
                <div>
                    <h1 class="text-2xl font-black text-slate-800 flex items-center gap-2">
                        <BriefcaseIcon class="w-8 h-8 text-blue-600" />
                        <span>Jadwal Mengajar</span>
                    </h1>
                    <p class="text-slate-500 font-medium text-sm">Lihat jadwal mengajar mingguan.</p>
                </div>
            </div>
                ...
                <div class="flex items-center bg-white p-1 rounded-xl border border-slate-200 shadow-sm">
                    <Select v-model="selectedTeacherId">
                        <SelectTrigger
                            ...
                            <span class="text-xs text-slate-400 mr-2 font-medium">Guru:</span>
                            <SelectValue />
                        </SelectTrigger>
                        <SelectContent>
                                    ...
                                    <div class="h-full w-full rounded-lg p-2 flex flex-col gap-1 border border-l-4 shadow-sm"
                                        ...
                                        <div
                                            ...
                                            <span class="text-[10px] font-bold opacity-80 truncate max-w-[100px]">
                                                Kelas {{ scheduleGrid[period.id][day].className }}
                                            </span>
                                        </div>
                                    </div>
                                </td>
